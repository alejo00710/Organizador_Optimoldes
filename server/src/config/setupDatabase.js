...
async function initializeDatabase() {
    ...
    try {
        ...
        // planning_history.client_name (cliente asociado al ciclo)
        try {
            const col = await query(
                `SELECT COUNT(1) AS cnt
                 FROM information_schema.columns
            ...
            const hasCol = Number(col?.[0]?.cnt || 0) > 0;
            if (!hasCol) {
                await query(`ALTER TABLE planning_history ADD COLUMN client_name VARCHAR(255) NULL`);
                console.log('✅ Migración aplicada: planning_history.client_name');
            }
        } catch (e) {
            console.warn('⚠️ No se pudo aplicar migración planning_history.client_name:', e.message);
        }
            ...
            await query(
                `CREATE TABLE IF NOT EXISTS user_sessions (
                  ...
                  operator_id INTEGER NULL REFERENCES operators(id) ON DELETE SET NULL,
                                    role TEXT NOT NULL CHECK (role IN ('admin','planner','management','operator')),
                  ip VARCHAR(64) NULL,
        ...
        // Asegurar checks de roles para bases ya existentes
        try {
            await query(`ALTER TABLE users DROP CONSTRAINT IF EXISTS users_role_check`);
            await query(
                `ALTER TABLE users
                 ADD CONSTRAINT users_role_check
                 CHECK (role IN ('admin','planner','management','operator'))`
            );
        } catch (e) {
            console.warn('⚠️ No se pudo actualizar users.role CHECK:', e.message);
        }

        try {
            await query(`ALTER TABLE user_sessions DROP CONSTRAINT IF EXISTS user_sessions_role_check`);
            await query(
                `ALTER TABLE user_sessions
                 ADD CONSTRAINT user_sessions_role_check
                 CHECK (role IN ('admin','planner','management','operator'))`
            );
        } catch (e) {
            console.warn('⚠️ No se pudo actualizar user_sessions.role CHECK:', e.message);
        }
    ...
    // El alta inicial de "admin", "jefe" y "gerencia" se hace vía Bootstrap en el login.
}
